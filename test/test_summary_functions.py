...
def test_mean():
    ...
    # grouped summarize
    t = df >> group_by(X.cut) >> summarize(m=mean(X.x))
    ...
    assert t.equals(df_truth)
    # grouped mutate
    t = df >> group_by(X.cut) >> mutate(m=mean(X.x))
...
def test_first():
    ...
    assert t.equals(df_truth)
    # grouped summarize
    t = df >> group_by(X.cut) >> summarize(f=first(X.x))
    df_truth = pd.DataFrame({'cut': ['Good', 'Ideal', 'Premium'],
                             'f': [4.05, 3.95, 3.89]})
    assert t.equals(df_truth)
    ...
    assert t.equals(df_truth)
    # grouped mutate
    t = df >> group_by(X.cut) >> mutate(f=first(X.x))
...
def test_last():
    ...
    assert t.equals(df_truth)
    # grouped summarize
    t = df >> group_by(X.cut) >> summarize(l=last(X.x))
    df_truth = pd.DataFrame({'cut': ['Good', 'Ideal', 'Premium'],
                             'l': [4.34, 3.95, 4.20]})
    assert t.equals(df_truth)
    # summarize with order_by
    ...
    assert t.equals(df_truth)
    # grouped mutate
    t = df >> group_by(X.cut) >> mutate(l=last(X.x))
...
def test_nth():
    ...
    assert t.equals(df_truth)
    # grouped summarize
    t = df >> group_by(X.cut) >> summarize(first=nth(X.x, 0))
    ...
    assert t.equals(df_truth)
    # grouped mutate
    t = df >> group_by(X.cut) >> mutate(penultimate=nth(X.x, -2))
    df_truth = df.copy()
...
def test_n():
    ...
    assert t.equals(df_truth)
    # grouped summarize
    t = df >> group_by(X.cut) >> summarize(n=n(X.x))
    df_truth = pd.DataFrame({'cut': ['Good', 'Ideal', 'Premium'],
                             'n': [2, 1, 2]})
    assert t.equals(df_truth)
    # straight mutate
    ...
    assert t.equals(df_truth)
    # grouped mutate
    t = df >> group_by(X.cut) >> mutate(n=n(X.x))
...
def test_n_distinct():
    ...
    assert t.equals(df_truth)
    # grouped summarize
    t = df >> group_by(X.col_1) >> summarize(n=n_distinct(X.col_2))
    ...
    assert t.equals(df_truth)
    # grouped mutate
    t = df >> group_by(X.col_1) >> mutate(n=n_distinct(X.col_2))
...
def test_IQR():
    ...
    assert t.equals(df_truth)
    # grouped summarize
    t = df >> group_by(X.cut) >> summarize(i=IQR(X.x))
    df_truth = pd.DataFrame({'cut': ['Good', 'Ideal', 'Premium'],
                             'i': [0.145, 0.000, 0.155]})
    ...
    assert t.equals(df_truth)
    # grouped mutate
    t = df >> group_by(X.cut) >> mutate(i=IQR(X.x))
...
def test_colmin():
    ...
    assert t.equals(df_truth)
    # grouped summarize
    t = df >> group_by(X.cut) >> summarize(m=colmin(X.x))
    df_truth = pd.DataFrame({'cut': ['Good', 'Ideal', 'Premium'],
                             'm': [4.05, 3.95, 3.89]})
    assert t.equals(df_truth)
    # straight mutate
    ...
    assert t.equals(df_truth)
    # grouped mutate
    t = df >> group_by(X.cut) >> mutate(m=colmin(X.x))
...
def test_colmax():
    ...
    assert t.equals(df_truth)
    # grouped summarize
    t = df >> group_by(X.cut) >> summarize(m=colmax(X.x))
    df_truth = pd.DataFrame({'cut': ['Good', 'Ideal', 'Premium'],
                             'm': [4.34, 3.95, 4.20]})
    assert t.equals(df_truth)
    # straight mutate
    ...
    assert t.equals(df_truth)
    # grouped mutate
    print(df.groupby('cut')['x'].agg(np.max))
    print(df)
    t = df >> group_by(X.cut) >> mutate(m=colmax(X.x))
...
def test_median():
    df = diamonds >> group_by(X.cut) >> head(3) >> select(X.cut, X.x) >> ungroup()
    # straight summarize
    ...
    # grouped summarize
    t = df >> group_by(X.cut) >> summarize(m=median(X.x))
    df_truth = pd.DataFrame({'cut': ['Fair', 'Good', 'Ideal', 'Premium', 'Very Good'],
    ...
    df = diamonds >> group_by(X.cut) >> head(2) >> select(X.cut, X.x)
    t = df >> group_by(X.cut) >> summarize(m=median(X.x))
    df_truth = pd.DataFrame({'cut': ['Fair', 'Good', 'Ideal', 'Premium', 'Very Good'],
...
def test_var():
    ...
    # grouped summarize
    t = df >> group_by(X.cut) >> summarize(v=var(X.x))
    df_truth = pd.DataFrame({'cut': ['Fair', 'Good', 'Ideal', 'Premium', 'Very Good'],
    ...
    df = diamonds >> group_by(X.cut) >> head(1) >> select(X.cut, X.x)
    t = df >> group_by(X.cut) >> summarize(v=var(X.x))
    df_truth = pd.DataFrame({'cut': ['Fair', 'Good', 'Ideal', 'Premium', 'Very Good'],
...
def test_sd():
    ...
    assert all(test_vector < .00001)
    # grouped summarize
    t = df >> group_by(X.cut) >> summarize(s=sd(X.x))
    df_truth = pd.DataFrame({'cut': ['Fair', 'Good', 'Ideal', 'Premium', 'Very Good'],
    ...
    assert all(test_vector < .00001)
    # grouped mutate
    t = df >> group_by(X.cut) >> mutate(s=sd(X.x))
